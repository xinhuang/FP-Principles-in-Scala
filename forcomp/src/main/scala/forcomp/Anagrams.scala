...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(c => c.toLower).map(pair => (pair._1, pair._2.length)).toList.sortBy(_._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s.map(wordOccurrences _).flatten
    .groupBy(_._1).map(p => (p._1, p._2.map(_._2).sum)).toList.sortBy(_._1)
