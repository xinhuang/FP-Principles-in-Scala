...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    explored: Set[Block]): Stream[(Block, List[Move])] = {
    val neighbors = initial.map(p => neighborsWithHistory(p._1, p._2)).flatten
    val newNeighbors = newNeighborsOnly(neighbors, explored)
    val newExplored = newNeighbors.map(_._1) ++ explored
    if (newNeighbors.size > 0)
      from(initial #::: newNeighbors, newExplored.toSet)
    else initial
  }
